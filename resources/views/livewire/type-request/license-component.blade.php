<div>
    <form method="POST" wire:submit.prevent="submit">
        ...
        <div class="col-md-12 mb-3 p-0">
            <label for="typeAction">Acción</label>
            <select name="typeAction" id="typeAction" wire:model="typeAction"
                class="form-control @error('typeAction') is-invalid @enderror">
                <option value="">Seleccione una acción</option>
                @foreach ($actions as $action)
                    <option value="{{ $action->COD }}">{{ $action->NOM }}</option>
                @endforeach
            </select>
            @error('typeAction')
